        ...
        for idx, (doc_id, cont_id) in enumerate(zip(batch["doc_id"], batch["cont_id"])):
            ...
            if self.metric_type == "pmi_dc":
                ...
                log_likelihood = (
                    ...
                    / torch.gather(
                        dc_lm_cont_logits, 1, cont_tokens.unsqueeze(-1)
                    ).sum()
                )
    ...
    def compute(self) -> torch.Tensor:
        ...
        for doc_id in loglikelihood_dict:
            ...
            if self.metric_type in ["ce_loss", "bpb"]:
                ...
                correct.append(
                    1.0
                    if torch.argmax(loglikelihoods).item() == label_dict[doc_id]
                    else 0.0
                )
...
class ICLMultiChoiceTaskDataset(metaclass=abc.ABCMeta):
    ...
    def __init__(
        ...
        if metric_type is not None:
            self.metric_type = metric_type
        self.log_instances = (
            0  # Set to > 0 to log the first few instances as a sanity check
        )
        ...
        for doc in self.dataset:
            for prompt in self.prompts:
                ...
                for cont_id, continuation_str in enumerate(continuations):
                    cont_str_len = (
                        len(continuation_str) - 1
                    )  # continuation contain leading blank
    ...
    def collate_fn(self, data):
        ...
        for sample in data:
            ...
            ctxs.append(
                torch.LongTensor(
                    self.pad_tokens_until_max(sample["ctx"], max_len=max_ctx_len)
                )
            )
            continuations.append(
                torch.LongTensor(
                    self.pad_tokens_until_max(
                        sample["continuation"], max_len=max_cont_len
                    )
                )
            )
            ...
            queries.append(
                torch.LongTensor(
                    self.pad_tokens_until_max(sample["query"], max_len=max_query_len)
                )
            )
            dc_queries.append(
                torch.LongTensor(
                    self.pad_tokens_until_max(
                        sample["dc_query"], max_len=max_dc_query_len
                    )
                )
            )
...
class WinoGrande(ICLMultiChoiceTaskDataset):
    ...
    def prep_examples(self):
        ...
        for doc in self.dataset:
            ...
            cont_str_len = (
                len(continuation_str) - 1
            )  # continuations contain leading blank space
            cont_byte_len = len(continuation_str[1:].encode("utf-8"))
                ...
                if config is not None:
                    ...
                    if metric_type_raw is not None:
                        ...
                        if (
                            ...
                            and self.metric_type != metric_type
                        ):
                            raise ValueError(
...
label_to_task_map = {
    ...
    "mmlu_social_sciences_test": (
        MMLU,
        {"dataset_name": "social_sciences", "split": "test"},
    ),
    ...
    "mmlu_humanities_var": (
        MMLU,
        {"dataset_name": "humanities", "prompt_variations": 1},
    ),
...
label_to_task_map_new = {
    ...
    "mmlu_humanities_val_rc_5shot": (
        MMLU,
        {"dataset_name": "humanities", "prompt_variations": 2},
    ),
    ...
    "mmlu_other_val_rc_var_bpb": (
        MMLU,
        {"dataset_name": "other", "prompt_variations": 1, "metric_type": "bpb"},
    ),
    "mmlu_other_val_rc_5shot": (
        MMLU,
        {"dataset_name": "other", "prompt_variations": 2},
    ),
