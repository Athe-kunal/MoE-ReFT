...
class DataCollator:
    ...
    def from_train_config(cls, config: TrainConfig) -> DataCollator:
        return cls(
            pad_direction=config.data.pad_direction,
            pad_token_id=config.model.pad_token_id,
        )

    def __call__(
        self, items: Union[List[Dict[str, Any]], List[torch.Tensor]]
    ) -> Dict[str, Any]:
        assert items
        max_len = max(
            (len(x["input_ids"] if isinstance(x, dict) else x) for x in items)
        )
        all_input_ids = []
        all_attention_mask = []
        all_attention_bias = []
        ...
        max_docs = max(
            (
                len(x["doc_lens"]) if isinstance(x, dict) and "doc_lens" in x else 0
                for x in items
            )
        )
        ...
        for x in items:
            ...
            if attention_bias is not None:
                ...
                pad_value = (
                    False if attention_bias.dtype == torch.bool else float("-inf")
                )
...
class CustomDatasetDataCollator(DataCollator):
    ...
    def _relabel_item(self, item: Dict[str, Any]) -> Dict[str, Any]:
        out = {
            ...
            "attention_mask": (
                item[self.attention_mask_field] if self.attention_mask_field else None
            ),
            "attention_bias": (
                item[self.attention_bias_field] if self.attention_bias_field else None
            ),
            "label_mask": (
                item[self.label_mask_field] if self.label_mask_field else None
            ),
            "index": item[self.index_field] if self.index_field else None,
            "instance_mask": (
                item[self.instance_mask_field] if self.instance_mask_field else None
            ),
