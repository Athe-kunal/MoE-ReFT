...
class IterableDataset(torch.utils.data.IterableDataset[Dict[str, Any]]):
    ...
    def __init__(
        self,
        dataset: Union[
            Sequence[List[int]], Sequence[torch.Tensor], Sequence[Dict[str, Any]]
        ],
        ...
        *,
        ...
        self.fs_local_rank = (
            fs_local_rank if fs_local_rank is not None else get_fs_local_rank()
        )
    ...
    def _build_and_save_global_indices(self):
        ...
        if self.fs_local_rank == 0:
            ...
            log.info(
                "Global data order indices saved to '%s'", self.global_indices_file
            )
        barrier()
    ...
    def __iter__(self) -> Iterator[Dict[str, Any]]:
        ...
        if worker_info is not None:
            ...
            truncated_size = self.device_batch_size * (
                len(indices) // self.device_batch_size
            )
            left_overs = indices[
        ...
        if num_threads:
            ...
            for i in range(num_threads):
                generator = (
                    self._get_dataset_item(int(idx)) for idx in indices[i::num_threads]
                )
