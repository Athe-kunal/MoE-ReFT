...
def build_collator(train_config: TrainConfig) -> DataCollator:
    ...
    if train_config.data.custom_dataset:
        if train_config.data.custom_dataset.collate_fn:
            module, function = extract_module_and_class(
                train_config.data.custom_dataset.collate_fn
            )
            if module is None:
                if train_config.data.custom_dataset.module is None:
                    module, _ = extract_module_and_class(
                        train_config.data.custom_dataset.name
                    )
                else:
...
def build_eval_dataloader(
    ...
    return DataLoader(
        ...
        prefetch_factor=(
            None if data_config.num_workers == 0 else data_config.prefetch_factor
        ),
        persistent_workers=(
            False if data_config.num_workers == 0 else data_config.persistent_workers
        ),
...
def build_train_dataloader(
    ...
    *,
    ...
    if train_config.data.custom_dataset:
        if (
            train_config.data.paths is not None
            or train_config.data.datasets is not None
        ):
            raise OLMoConfigurationError(
